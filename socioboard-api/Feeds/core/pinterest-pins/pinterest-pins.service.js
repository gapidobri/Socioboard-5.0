 ...
 */
  async getPinterestPins(req,res,next){
    try {
         ...
         const { value, error } = Validate.validatePinterstAcc({accountId,boardId, teamId, pageId});
         if (error) return ValidateErrorResponse(res, error.details[0].message);
         const socialAccountDetails = await userTeamAccounts.getSocialAccount(11,accountId,req.body.userScopeId,teamId);
         const pins = await  this.getPins(req.body.userScopeId,accountId,boardId,teamId,pageId);
 ...
 /**
  ...
  async getPins(userId, accountId,boardId, teamId, pageId) {
      ...
      return userTeamAccounts.isNeedToFetchRecentPost(
        accountId,
      ...
      .then(isRunRecentPost => {
              ...
              .then(socialAccountDetails => {
                ...
                return accountUpdateTable
                  ...
                  .then(updatedAccountData => {
                    ...
                    return Pincluster.getBoardPins(socialAccountInfo.social_id,boardId,socialAccountInfo.access_token)
                   .then((response)=>{
                    feeds.push(response)
               ...
               .then(() =>
                ...
                .then(() => feeds)
                .catch(error => {
                  logger.error(`Error while fetching Recent pinterest Pins ${error.message}`)
                  reject(error)
                  })
              )
              .catch(error => {
        ...
        .then(() => this.getPinsFromDB(userId, accountId,boardId, teamId, pageId))
        .then(feeds => {
           resolve(feeds)
          })
        .catch(error => {
            ...
            return this.getSocialAccount(11, accountId, userId, teamId)
                .then((socialAccount) => {
                    let offset = (pageId - 1) * config.get('perPageLimit');
                       let PinterestModel = new PinterestMongoPinsModel();
                      return PinterestModel.getPinterestPins(socialAccount.social_id,boardId, offset, config.get('perPageLimit'));
                })
